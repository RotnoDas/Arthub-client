...
const CommentSection = ({ artworkId, session, isPurchased }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/comments/${artworkId}`);
                if (res.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/artworks/${artworkId}/comments`, {
                method: 'POST',
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/comments/${itemToDelete}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const handleEditSave = async (commentId) => {
        ...
        try {
            const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/comments/${commentId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="mt-8 border-t border-border pt-8 transition-colors duration-500">
            <h3 className="text-2xl font-extrabold text-foreground mb-6 flex items-center gap-2">
            ...
            <div className="mb-8 p-6 bg-indigo-50/50 dark:bg-indigo-950/20 rounded-3xl border border-indigo-100 dark:border-indigo-900/50 shadow-sm transition-colors duration-500">
                {!session ? (
                    <div className="text-center py-6">
                        ...
                        <h4 className="text-lg font-bold text-foreground mb-2">Join the conversation</h4>
                        <p className="text-muted mb-6">You must log in to participate.</p>
                        <Link href="/login">
                    ...
                    <div className="text-center py-6">
                        ...
                        <h4 className="text-lg font-bold text-foreground mb-2">Verified Buyers Only</h4>
                        <p className="text-muted">Only users who have purchased this artwork can leave a comment.</p>
                    </div>
                ) : (
            ...
            <div className="space-y-4">
                {isLoading ? (
                    ...
                    <div className="text-center text-slate-400 dark:text-slate-500 py-10 font-medium bg-slate-50/50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-border transition-colors duration-500">
                                    ...
                                    {editingId === c._id ? (
                                        <div className="mt-2 space-y-2">
                                            <textarea
                                                ...
                                                rows={2}
                                                className="w-full bg-background text-foreground border-2 border-indigo-200 dark:border-indigo-900/50 focus:border-indigo-500 rounded-xl p-3 outline-none text-sm"
                                            />
                                        ...
                                        <p className="text-muted leading-relaxed whitespace-pre-wrap text-sm">{c.comment || c.text}</p>
                                    )}
                                </div>
                            </div>
                        );
