...
const ArtCategories = () => {
    return (
        <section className="py-24 relative overflow-hidden border-t border-slate-200">
            ...
            <div className="absolute top-0 right-1/4 w-96 h-96 bg-fuchsia-100/40 rounded-full blur-[100px] -z-10" />
            <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-indigo-100/40 rounded-full blur-[100px] -z-10" />
                    ...
                    {categories.map((category) => {
                        ...
                        return (
                            <Link href={`/artworks?category=${encodeURIComponent(category.name)}`} key={category.name}>
                                <div 
                                    className={`group flex flex-col items-center justify-center bg-slate-100 w-36 h-36 md:w-44 md:h-44 rounded-[2.5rem] border-2 border-slate-300 shadow-xl shadow-slate-300/50 hover:shadow-2xl hover:shadow-${category.color.split('-')[1]}-500/30 hover:-translate-y-2 transition-all duration-300 cursor-pointer relative overflow-hidden`}
                                >
                                    {/* Glass reflection */}
                                    ...
                                    <div className={`p-4 rounded-full bg-white shadow-md mb-3 group-hover:scale-110 transition-transform duration-300 ${category.border} border-2`}>
                                        <Icon className={`w-8 h-8 ${category.color}`} />
                                    </div>
                                    <span className="font-bold text-slate-800 text-sm md:text-base group-hover:text-slate-900 relative z-10">{category.name}</span>
                                </div>
                            </Link>
                        );
                    })}
