...
export default function PurchaseHistoryPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-1">Purchase History</h1>
        <p className="text-muted">Every artwork you've bought on ArtHub.</p>
      </div>

      <div className="rounded-2xl border border-border bg-background shadow-sm overflow-hidden transition-colors duration-500">
        {loading ? (
          <div className="p-12 text-center text-muted animate-pulse">Loading purchases...</div>
        ) : purchases.length === 0 ? (
          <div className="p-14 text-center">
            <FaPalette className="mx-auto text-5xl text-slate-300 dark:text-slate-700 mb-4" />
            <p className="text-muted font-medium text-lg">No purchases yet.</p>
