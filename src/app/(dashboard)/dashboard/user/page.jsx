...
export default function UserDashboard() {
  ...
  if (!user || loading) return <div className="text-muted p-8">Loading...</div>;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted mt-1">Your personal ArtHub collection dashboard.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {[
          ...
          <div key={s.label} className={`rounded-2xl border border-border bg-background p-5 flex items-center gap-4 hover:border-${s.color}-500/30 transition-colors shadow-sm`}>
            ...
            <div>
              ...
              <p className="text-foreground font-bold text-xl truncate max-w-[140px]">{s.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-border bg-background shadow-sm p-6 sm:p-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-[0.03] dark:opacity-[0.05] text-foreground">
          <FaCrown size={120} />
        </div>
      ...
      {/* Quick Nav Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <Link href="/dashboard/user/purchases" className="group rounded-2xl border border-border bg-background hover:bg-pink-50 dark:hover:bg-pink-500/10 hover:border-pink-200 dark:hover:border-pink-500/30 transition-all duration-200 p-6 flex flex-col gap-3 shadow-sm">
          ...
          <div>
            <p className="font-bold text-foreground">Purchase History</p>
            <p className="text-muted text-xs mt-1">View all your past artwork purchases.</p>
          </div>
        </Link>
        <Link href="/dashboard/user/collection" className="group rounded-2xl border border-border bg-background hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all duration-200 p-6 flex flex-col gap-3 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-indigo-100 dark:bg-indigo-500/10 text-accent-secondary flex items-center justify-center text-lg group-hover:bg-indigo-200 dark:group-hover:bg-indigo-500/20 transition-colors">
            <FaPalette />
          </div>
          <div>
            <p className="font-bold text-foreground">My Collection</p>
            <p className="text-muted text-xs mt-1">Browse your gallery of owned artworks.</p>
          </div>
        </Link>
        <Link href="/dashboard/user/profile" className="group rounded-2xl border border-border bg-background hover:bg-purple-50 dark:hover:bg-purple-500/10 hover:border-purple-200 dark:hover:border-purple-500/30 transition-all duration-200 p-6 flex flex-col gap-3 shadow-sm">
          ...
          <div>
            <p className="font-bold text-foreground">Profile Settings</p>
            <p className="text-muted text-xs mt-1">Edit your profile and change password.</p>
          </div>
        </Link>
      </div>
      ...
      <div>
        ...
        <div className="rounded-2xl border border-border bg-background shadow-sm overflow-hidden">
          {loading ? (
            <div className="p-10 text-center text-muted animate-pulse">Loading...</div>
          ) : purchases.length === 0 ? (
            <div className="p-10 text-center">
              <FaPalette className="mx-auto text-4xl text-slate-300 dark:text-slate-700 mb-3" />
              <p className="text-muted font-medium">No purchases yet.</p>
