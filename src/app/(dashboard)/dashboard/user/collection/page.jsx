...
export default function CollectionPage() {
  ...
  useEffect(() => {
    if (user?.email) {
      apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/artworks/purchase/${user.email}`)
        .then((res) => res.json())
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-1">My Collection</h1>
        <p className="text-muted">A gallery of all the artworks you own.</p>
      </div>

      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="rounded-2xl border border-border bg-slate-50 dark:bg-slate-900/50 aspect-square animate-pulse" />
          ))}
        </div>
      ) : purchases.length === 0 ? (
        <div className="rounded-2xl border border-border bg-background shadow-sm p-16 text-center transition-colors duration-500">
          <FaPalette className="mx-auto text-5xl text-slate-300 dark:text-slate-700 mb-4" />
          <p className="text-foreground font-semibold text-lg">Your collection is empty</p>
          <p className="text-muted text-sm mt-2 mb-6">Discover and collect unique artworks from talented artists.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {purchases.map((item) => (
            <div key={item._id} className="group rounded-2xl border border-border bg-background shadow-sm hover:border-pink-300 dark:hover:border-pink-500/50 hover:shadow-lg hover:shadow-pink-500/5 dark:hover:shadow-pink-500/10 transition-all duration-300 overflow-hidden">
              ...
              {/* Card Info */}
              <div className="p-4">
                <p className="font-bold text-foreground text-sm truncate group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">{item.artworkTitle}</p>
                <p className="text-muted text-xs mt-0.5 truncate">by {item.artistName || item.artistEmail}</p>
      ...
      {purchases.length > 0 && (
        <p className="text-muted text-sm text-center">{purchases.length} artwork{purchases.length !== 1 ? "s" : ""} in your collection</p>
      )}
    </div>
  );
}
