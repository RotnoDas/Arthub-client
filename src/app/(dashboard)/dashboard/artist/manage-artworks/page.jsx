  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-1">Manage Artworks</h1>
          <p className="text-slate-500">Edit, delete, or review your listed artworks.</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-14 text-center">
            ...
            <p className="text-slate-500 font-semibold text-lg">No artworks yet</p>
            <p className="text-slate-400 text-sm mt-1 mb-5">Post your first artwork and start selling.</p>
            <Link href="/dashboard/artist/add-artwork">
                ...
                <tr key={art._id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                  <td className="px-5 py-3.5">
                    {art.image
                      ...
                      : <div className="w-12 h-12 rounded-xl bg-indigo-50 flex items-center justify-center"><FaPalette className="text-indigo-600" /></div>
                    }
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-green-600 font-bold">${art.price}</td>
                  <td className="px-5 py-3.5">
                    <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${art.status === "sold" ? "bg-green-100 text-green-600" : "bg-blue-100 text-blue-600"}`}>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <Link href={`/dashboard/artist/add-artwork?edit=${art._id}`}>
                        <button className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center justify-center transition-colors">
                          <FaEdit size={13} />
                        </button>
                      </Link>
                      <button onClick={() => handleDelete(art._id)} className="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center transition-colors">
                        <FaTrash size={13} />
                      </button>
                    </div>
                  </td>
