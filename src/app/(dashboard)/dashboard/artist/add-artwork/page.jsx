...
export default function AddArtworkPage() {
  ...
  useEffect(() => {
    if (editId) {
      apiFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/single-artworks/${editId}`)
        .then(async (r) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editId ? `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/artworks/${editId}` : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/artworks`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl">
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-1">{editId ? "Edit Artwork" : "Add New Artwork"}</h1>
        <p className="text-muted">{editId ? "Update your artwork details below." : "List a new artwork on the ArtHub marketplace."}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Image Upload */}
        <div className="rounded-2xl border-2 border-dashed border-border bg-slate-50 dark:bg-slate-900/50 p-6 text-center hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-colors duration-500">
          {formData.image ? (
            <div className="space-y-4">
              <img src={formData.image} alt="Preview" className="w-full max-h-64 object-contain rounded-xl mx-auto" />
              <div className="flex gap-3 justify-center">
                <label className="cursor-pointer px-4 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 text-accent-secondary text-sm font-semibold hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition flex items-center gap-2">
            ...
            <label className="cursor-pointer block">
              ...
              <div className="space-y-3 py-4">
                <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 text-accent-secondary flex items-center justify-center text-2xl mx-auto transition-colors">
                  <FaUpload />
                </div>
                <div>
                  <p className="text-foreground font-semibold transition-colors">{uploading ? "Uploading..." : "Upload Artwork Image"}</p>
                  <p className="text-muted text-xs mt-1 transition-colors">Click to browse · PNG, JPG, WEBP · Max 10MB</p>
                </div>
        ...
        {/* Title */}
        <div>
          <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Artwork Title *</label>
        ...
        <div className="grid grid-cols-2 gap-5">
          <div>
            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Price (USD) *</label>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => router.push("/dashboard/artist/manage-artworks")} className="px-6 py-2.5 rounded-xl border border-border text-muted hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50 text-sm font-semibold transition-colors flex items-center gap-2">
