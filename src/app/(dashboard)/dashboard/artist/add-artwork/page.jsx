...
export default function AddArtworkPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl">
      <div>
        <h1 className="text-3xl font-bold text-white mb-1">{editId ? "Edit Artwork" : "Add New Artwork"}</h1>
        <p className="text-slate-400">{editId ? "Update your artwork details below." : "List a new artwork on the ArtHub marketplace."}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Image Upload */}
        <div className="rounded-2xl border-2 border-dashed border-white/10 bg-white/5 p-6 text-center hover:border-indigo-500/40 transition-colors">
          {formData.image ? (
            <div className="space-y-4">
              <img src={formData.image} alt="Preview" className="w-full max-h-64 object-contain rounded-xl mx-auto" />
              <div className="flex gap-3 justify-center">
                <label className="cursor-pointer px-4 py-2 rounded-xl bg-indigo-500/10 text-indigo-400 text-sm font-semibold hover:bg-indigo-500/20 transition flex items-center gap-2">
                  <FaUpload size={12} /> Change Image
                  <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                </label>
                <button type="button" onClick={() => setFormData(p => ({ ...p, image: "" }))} className="px-4 py-2 rounded-xl bg-red-500/10 text-red-400 text-sm font-semibold hover:bg-red-500/20 transition flex items-center gap-2">
            ...
            <label className="cursor-pointer block">
              ...
              <div className="space-y-3 py-4">
                <div className="w-14 h-14 rounded-2xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center text-2xl mx-auto">
                  <FaUpload />
                </div>
                <div>
                  <p className="text-white font-semibold">{uploading ? "Uploading..." : "Upload Artwork Image"}</p>
                  <p className="text-slate-500 text-xs mt-1">Click to browse · PNG, JPG, WEBP · Max 10MB</p>
                </div>
                {uploading && <div className="w-48 h-1.5 bg-white/10 rounded-full mx-auto overflow-hidden"><div className="h-full bg-indigo-500 rounded-full animate-pulse w-3/4" /></div>}
              </div>
            </label>
          )}
        </div>
        ...
        {/* Title */}
        <div>
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Artwork Title *</label>
        ...
        <div className="grid grid-cols-2 gap-5">
          <div>
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Price (USD) *</label>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => router.push("/dashboard/artist/manage-artworks")} className="px-6 py-2.5 rounded-xl border border-white/10 text-slate-400 hover:text-white hover:bg-white/5 text-sm font-semibold transition-colors flex items-center gap-2">
