  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-1">Sales History</h1>
          <p className="text-muted">All purchases made on your artworks.</p>
        </div>
        {sales.length > 0 && (
          <div className="rounded-2xl border border-green-200 dark:border-green-500/30 bg-green-50 dark:bg-green-500/10 px-5 py-3 text-right shadow-sm">
            <p className="text-xs text-muted font-bold uppercase tracking-wider">Total Earned</p>
      ...
      <div className="rounded-2xl border border-border bg-background shadow-sm overflow-hidden transition-colors duration-500">
        {loading ? (
          <div className="p-12 text-center text-muted animate-pulse">Loading sales...</div>
        ) : sales.length === 0 ? (
          <div className="p-14 text-center">
            <FaHistory className="mx-auto text-5xl text-slate-300 dark:text-slate-700 mb-4" />
            <p className="text-muted font-semibold text-lg">No sales yet</p>
      ...
      {sales.length > 0 && (
        <p className="text-muted text-sm px-1">{sales.length} sale{sales.length !== 1 ? "s" : ""} total</p>
      )}
    </div>
  );
}
