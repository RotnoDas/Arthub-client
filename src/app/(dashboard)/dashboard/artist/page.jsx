...
export default function ArtistDashboardOverview() {
  ...
  if (!user) return <div className="text-slate-500 dark:text-slate-400 p-8">Loading...</div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Artist Studio, {user.name} 🎨</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your portfolio and track your sales.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        {[
          ...
          <div key={s.label} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 flex items-center gap-4 hover:border-slate-300 dark:hover:border-slate-700 transition-colors shadow-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="text-slate-900 dark:text-white font-bold text-xl truncate">{s.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          ...
          <Link key={c.href} href={c.href} className={`group rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-${c.color}-50 dark:hover:bg-${c.color}-500/10 hover:border-${c.color}-200 dark:hover:border-${c.color}-500/30 transition-all duration-200 p-6 flex flex-col gap-3 shadow-sm`}>
            ...
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs mt-1">{c.desc}</p>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Recent Artworks</h2>
          <Link href="/dashboard/artist/manage-artworks" className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">View All →</Link>
        </div>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
          {loading ? (
            <div className="p-10 text-center text-slate-500 dark:text-slate-400 animate-pulse">Loading artworks...</div>
          ) : artworks.length === 0 ? (
            <div className="p-10 text-center">
              <FaPalette className="mx-auto text-4xl text-slate-300 dark:text-slate-700 mb-3" />
              <p className="text-slate-500 dark:text-slate-400 font-medium">No artworks posted yet.</p>
            ...
            <div className="overflow-x-auto">
              ...
              <tbody>
                {artworks.slice(0, 5).map(a => (
                  <tr key={a._id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-5 py-3.5 font-semibold text-slate-900 dark:text-white flex items-center gap-3">
                      {a.image ? <img src={a.image} alt={a.title} className="w-9 h-9 rounded-lg object-cover shrink-0" /> : <div className="w-9 h-9 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center shrink-0"><FaPalette className="text-indigo-600 dark:text-indigo-400 text-xs" /></div>}
                      <span className="truncate max-w-[180px]">{a.title}</span>
                    </td>
                    <td className="px-5 py-3.5 text-slate-600 dark:text-slate-400">{a.category || "—"}</td>
