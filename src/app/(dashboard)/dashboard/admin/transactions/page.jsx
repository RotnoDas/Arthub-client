...
export default function AdminTransactionsPage() {
  ...
  const typeBadge = (t) => {
    ...
    if (type.includes("subscription")) return "bg-fuchsia-50 dark:bg-fuchsia-500/10 text-accent border border-fuchsia-100 dark:border-fuchsia-500/20";
    ...
    return "bg-slate-50 dark:bg-slate-800 text-muted border border-border";
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-1">All Transactions</h1>
          <p className="text-muted">Every payment processed on the ArtHub platform.</p>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-background shadow-sm overflow-hidden transition-colors duration-500">
        {loading ? (
          <div className="p-12 text-center text-muted animate-pulse">Loading transactions...</div>
        ) : filtered.length === 0 ? (
          <div className="p-14 text-center">
            <FaExchangeAlt className="mx-auto text-5xl text-slate-300 dark:text-slate-700 mb-4" />
            <p className="text-muted font-medium">{search ? "No results found." : "No transactions yet."}</p>
          </div>
        ) : (
