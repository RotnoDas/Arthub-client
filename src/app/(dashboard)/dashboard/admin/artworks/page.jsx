  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-1">All Artworks</h1>
          <p className="text-muted">Moderate and manage every artwork on the platform.</p>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-background shadow-sm overflow-hidden transition-colors duration-500">
        {loading ? (
          ...
          <div className="p-14 text-center">
            ...
            <p className="text-muted font-medium">{search ? "No artworks match your search." : "No artworks found."}</p>
          </div>
        ) : (
                  ...
                  <tr key={art._id} className="border-b border-border hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        {art.image
                          ? <img src={art.image} alt={art.title} className="w-11 h-11 rounded-xl object-cover shrink-0 border border-border" />
                        ...
                        <span className="font-semibold text-foreground truncate max-w-[150px]">{art.title}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-muted text-xs">{art.artistName || art.artistEmail}</td>
      ...
      {!loading && <p className="text-muted text-sm px-1">{filtered.length} of {artworks.length} artwork{artworks.length !== 1 ? "s" : ""}</p>}
